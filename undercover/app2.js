...
(function () {
  ...
  /* ----------------------------------------------------------
     13. RADAR INTERACTIVE INTERFACE (Option 2)
     ---------------------------------------------------------- */
  const radarPanel = document.getElementById('radar-panel');
  const radarGlow = document.getElementById('radar-glow');
  const radarBeam = document.getElementById('radar-beam');
  const radarLabels = document.querySelectorAll('.radar-hover-label');
  const lockedCard = document.getElementById('radar-locked-card');

  if (radarPanel) {
    let lockTimeout = null;

    radarPanel.addEventListener('mouseenter', () => {
      if (radarBeam) {
        radarBeam.style.opacity = '0.85';
      }
      // Start 1.5s lock-in timeout
      clearTimeout(lockTimeout);
      lockTimeout = setTimeout(() => {
        if (lockedCard) {
          lockedCard.classList.add('locked');
        }
      }, 1500);
    });

    radarPanel.addEventListener('mousemove', (e) => {
      const rect = radarPanel.getBoundingClientRect();
      // Mouse coordinates relative to the panel container
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;

      // Update cursor spotlight position
      if (radarGlow) {
        radarGlow.style.left = `${x}px`;
        radarGlow.style.top = `${y}px`;
      }

      // Update vertical scanning line position
      if (radarBeam) {
        radarBeam.style.left = `${x}px`;
      }

      // Check proximity of cursor to each hidden label
      radarLabels.forEach(lbl => {
        // Compute label center position relative to its offsetParent (.radar-panel)
        const lblX = lbl.offsetLeft + lbl.offsetWidth / 2;
        const lblY = lbl.offsetTop + lbl.offsetHeight / 2;
        const dist = Math.hypot(x - lblX, y - lblY);

        // If distance is less than 110px, reveal it; otherwise hide
        if (dist < 110) {
    ...
    radarPanel.addEventListener('mouseleave', () => {
      if (radarBeam) {
        radarBeam.style.opacity = '0';
      }
      // Clear timeout & hide locked card
      clearTimeout(lockTimeout);
      if (lockedCard) {
    ...
    radarPanel.addEventListener('touchstart', (e) => {
      e.preventDefault(); 
      if (radarBeam) {
        radarBeam.style.opacity = '0.85';
      }
    ...
    radarPanel.addEventListener('touchend', () => {
      if (radarBeam) {
        radarBeam.style.opacity = '0';
      }
      clearTimeout(lockTimeout);
